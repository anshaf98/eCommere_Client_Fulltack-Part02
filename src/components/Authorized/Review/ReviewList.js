  ...
  reviews &&
    reviews.forEach((rev) => {
      rows.push({
        id: rev?._id,
        image:
          IMAGE_BASEURL + rev?.user?.avatar?.url
            ? IMAGE_BASEURL + rev?.user?.avatar?.url
            : `https://png.pngtree.com/png-vector/20220623/ourmid/pngtree-user-avatar-icon-profile-silhouette-png-image_5173766.png`,
        name: rev?.user?.name || "User",
        rating: rev?.rating,
        review: rev?.comment,
      });
    });
